...
knitr::opts_knit$set(root.dir = normalizePath("C:/Users/13063/Documents/GitHub/stats506"))
#libraries:
#library(SASxport)
library(tidyverse)
library(data.table)
...
setwd("C:/Users/13063/Documents/GitHub/stats506")
...
``{r, echo = TRUE, eval=FALSE}
 /* 
Stats 506, Fall 2019
 Group project, SAS version: Karthik G
 ...
 of salt intake behaviour on the blood pressure (both diastole and systole) of 
 a person with waist size and age acting as moderator and mediator respectively. 
 Author: Karthik G (gkarthik@umich.edu)
 Date: December 11, 2019
*/
```
...
We have independent variable which is the frequency of add salt to food 
at table represented the salt intake behavior of each subject.
We can either treat this as a categorical variable with three level
("Rarely", "Occasionally" or "Very often" to add salt), or we can treat 
this variable as numeric variable represented the relative frequency of 
add salt to food.

For R and SAS, we impliment the analysis methods treated variable "the frequency of add salt to food 
at table" as numeric variable.
For Python, we impliment the analysis methods treated variable "the frequency of add salt to food 
at table" as categorical variable.
Treat "the frequency of add salt to food at table" as categorical variable
may lose some information on the on relative salt intake frequency difference between subject,
while treated it as numeric variable may lead to misspecified frequency difference between subject.

For R and SAS the results of analysis are same. The conclusion of all three different implimentation
<<<<<<< HEAD
are the same. There are some difference between R, SAS and Python impliment since we treated
independent variable differently and add extra degrees of freedom.
However, if our independent variable is of two levels, 
treating it as categorical variable or as numeric variable are essentially the same.  

When fit least square to model `DI ~ DBD100`, the coefficient of numeric variable is significant
...
and blood pressure, all analysis show that the interactions between waist size and salt intake are
...
do not have significant influence on blood pressure. But this difference will not affect the result that
...
=======
are the same. However, there are some difference between R, SAS and Python impliment since we treated
independent variable differently.
<br>
<br>
>>>>>>> 594e706f3619a6fa22eef9212034fd32fcdaca72
