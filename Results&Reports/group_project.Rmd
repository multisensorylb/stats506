...
For R and SAS, we implemented the analysis methods by treating the variable 
"the frequency of add salt to food at table" as numeric.
For Python, we implemented the analysis methods by treating the variable 
"the frequency of add salt to food at table" as categorical.
Treating "the frequency of add salt to food at table" as categorical variable
may lose some information on the relative salt intake frequency difference between subject,
while treating it as a numeric variable may lead to misspecified frequency difference between subject.

For R and SAS the results of the analysis are same. The conclusion obtained from all three different 
implementation are the same. There are some differences between R, SAS and Python implementations since we 
treated independent variables differently and added extra degrees of freedom.
...
and blood pressure, all analysis showed that the interactions between waist size and salt intake are
