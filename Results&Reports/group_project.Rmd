...
Moderator Z is a variable that can affect the direction or strength of the relationship between X and Y. If the effect of salt intake on the blood pressure depends on the level of waist size, then waist size is a moderator. Otherwise, blood pressure is not a moderator.

We perform moderation analysis by adding an interaction term, which can be expressed as $y = \beta_0 + \beta_1 x + \beta_2 z + \beta_3 x * z + \epsilon$.
If $\beta_3$ does not equal to 0, Z is supposed to be a moderator. In the regression model, we test whether the regression coefficient for the interaction term is significant or not.

Mediator explains the underlying mechanism of the relationship between the independent variable and the dependent variable. Let’s assume that the independent variable X is a predictor for the dependent variable Y and the relationship is that X boosts Y. If M is a mediator, then the hypothesis is that X boosts M and then M boosts Y. Three steps of regression comprise a mediation analysis: X to Y, X to M, X+M to Y.
...
The residuals of diastolic have two thick tails compares to normal.
The residuals of systolic have a thick right tail compares to normal.
...
![Fig: Structural model for the moderation analysis](figures_for_report/fig1.png)
...
![Fig: Direct and indirect effects of the mediation analysis for Diastole ](figures_for_report/fig2.png)
...
![Fig: Direct and indirect effects of the mediation analysis for Diastole ](figures_for_report/fig3.png)
...
We need to first clean the raw data and join different datasets.
...
From the plot shown above, we know that the distribution of blood pressure given salt intake is approximately normal with a thick left tail.


```python
# Test normality of Systolic given salt intake
ax1 = plt.subplot(131)
a=SY_salt[['1']].dropna().to_numpy()
a=np.reshape(a,(len(a)))
qqplot = stats.probplot(a, plot=plt)
plt.yticks([])

ax2 = plt.subplot(132)
b=SY_salt[['2']].dropna().to_numpy()
b=np.reshape(b,(len(b)))
qqplot = stats.probplot(b, plot=plt)
plt.yticks([])

ax3 = plt.subplot(133)
c=SY_salt[['3']].dropna().to_numpy()
c=np.reshape(c,(len(c)))
qqplot = stats.probplot(c, plot=plt)
plt.yticks([])
...
The distribution of blood pressure given salt intake is approximately normal with a thick right tail.
...
From the above results, we know that both models are significant and every coefficient is significant(at the level of 95%).

### Moderation effect of waist size

First, add two columns recording a standard deviation above and below of waist size.
...
For the models above, coefficients for the interaction terms and salt intake itself are not significant(at the level of 95%).
There is no moderation effect of waist size on salt intake and diastole.
...
All the mediation effect(ACME) are significant(at the level of 95%).
This means that age is a mediator between salt intake and diastolic.
...
Even thought the coefficients of salt intake is not significant(at the level of 95%).
...
That means that age is a mediator between salt intake and systolic.
...
salt intake as a categorical variable.
...
There is no moderation effects of waist size on salt intake and diastole.
...
The model is significant. 
Even though the coefficients of salt intake is not significant(at the level of 95%).
Age might be a mediator between salt intake and systolic.
...
All the mediation effect(ACME) are significant.
This means that age is a mediator between salt intake and systolic.

#### Compare the difference between treating independent variable as categorical variable and as numeric variable

From the analysis above, we know that salt intake behavior has a significant influence on people’s blood pressure (both diastolic and systolic). The influence of salt intake behavior on blood pressure(both diastolic and systolic) is not moderated by waist size. Age is a Mediator between salt intake behavior and blood pressure(both diastolic and systolic).

Our independent variable is the salt intake behavior of each subject. We can either treat this as a categorical variable with three levels (“Rarely”, “Occasionally” or “Very often” to add salt), or we can treat this variable as numeric, representing the relative frequency of adding salt to food at the table.

The conclusions obtained from treated salt intake differently are the same. There are some differences exist since we added extra degrees of freedom. However, if our independent variable is of two levels, treating it as a categorical variable or as a numeric variable is essentially the same.

When we fit the least square model for DI ~ DBD100, the coefficient of the numeric variable is significant (for both diastolic and systolic). The two coefficients of factor variable of 3 levels are both significant (for both diastolic and systolic), and the effect of the independent variable on blood pressure are significant from the result of ANOVA analysis.

When we analyze the moderation effect of waist size on the relationship between salt intake behavior and blood pressure, both analyses showed that the interactions between waist size and salt intake are not significant. The main difference is that ANOVA of categorical data shows that salt intake behavior has a significant influence on blood pressure, while regression results show that salt intake behavior does not have a significant influence on blood pressure. The t-tests for each dummy variable are not significant while F-test is significant. But this difference will not affect the result since there is no moderation effect of waist size.
